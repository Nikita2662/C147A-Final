...
@dataclass
class AugmentedCopiesDataset(torch.utils.data.Dataset):
    """
    Wraps a base dataset and returns the original sample plus extra
    augmented copies of each sample.
    """

    base_dataset: torch.utils.data.Dataset
    extra_augment_transform: Transform[torch.Tensor, torch.Tensor] | None = None
    copies_per_sample: int = 0

    def __len__(self) -> int:
        return len(self.base_dataset) * (self.copies_per_sample + 1)

    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        group_size = self.copies_per_sample + 1
        base_idx = idx // group_size
        copy_idx = idx % group_size

        emg, labels = self.base_dataset[base_idx]

        if copy_idx > 0 and self.extra_augment_transform is not None:
            emg = self.extra_augment_transform(emg)
