...
class AdditiveGaussianNoise:
    """
    Adds Gaussian noise elementwise. Mimics sensor/electronics noise and small motion artifacts.
    """
    std: float = 0.01
    p: float = 0.5

    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        if self.p <= 0 or np.random.rand() > self.p or self.std <= 0:
            return x
...
class RandomGain:
    """
    Random amplitude scaling. Mimics differences in strap tightness, skin impedance,
    day-to-day calibration, and muscle effort.
    """
    min_gain: float = 0.8
    max_gain: float = 1.2
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # broadcast gain over all dims except channel dim (-1)
        shape = [1] * x.ndim
        shape[-1] = x.shape[-1]
        g = torch.empty(shape, device=x.device, dtype=x.dtype).uniform_(
...
@dataclass
class RandomBandGain:
    """
    Randomly scales each EMG band independently. Mimics differences in strap tightness, skin impedance,
    and contact quality between the left and right wrist bands.
    """
    min_gain: float = 0.85
    max_gain: float = 1.15
    p: float = 0.5
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B = x.shape[self.band_dim]

        # Create one random gain per band
        shape = [1] * x.ndim
        shape[self.band_dim] = B

        gains = torch.empty(shape, device=x.device, dtype=x.dtype).uniform_(
            self.min_gain, self.max_gain
        )
